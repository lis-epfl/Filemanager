...
Connector.editfile = function(params, callback) {
	var content = FS.readFile(params.path, { encoding : "utf8" });
	callback({
		"Error" : "No Error",
		"Code" : 0,
		"Path" : params.path,
...
Connector.deleteSync = function(file) {
	file = file.replace(/\/+$/, "");
	console.log("stating " + file);
	var info = FS.stat(file);
	var isDir = FS.isDir(info.mode);
	if (isDir) {
		console.log("recursive delete " + file);
		var content = FS.readdir(file);
		console.log(content);
		for (var i in content) {
			var fName = content[i];
			if (fName != "." && fName != "..") {
				var innerFile = file + "/" + content[i];
				Connector.deleteSync(innerFile);
			}
		}
		FS.rmdir(file);
	} else {
...
Connector.delete = function(params, callback) {
	var file = params.path;
	Connector.deleteSync(file);
	Connector.sync(callback.bind(null, {
		Error : "No Error",
		Path : file,
		Code : 0
	}));
...
Connector.getInfoSync = function(realpath) {
		realpath = realpath.replace(/\/+$/, "");
		var info = FS.stat(realpath);
		var isDir = FS.isDir(info.mode);
		var extension = "";
		...
		var res = {};
		res.Filename = realpath.split("/").pop();
		if (isDir) {
			realpath += "/";
			res["File Type"] = "dir";
			res.Preview = "images\/fileicons\/_Open.png";
		} else {
			res.Preview = "images\/fileicons\/"+extension+".png";
			res["File Type"] = extension;
		}
		res.Path = realpath;
		res.Protected = 0;
		res.Properties = {
			"Date Created": info.ctime,
			"Date Modified": info.mtime,
			"filemtime": info.mtime.getTime(),
			"Height":null,
			"Width":null,
			"Size": info.size
		}
...
Connector.sync = function(callback) {
	FS.syncfs(function() {
		console.log("SYNCED");
		if(callback) {
			callback();
...
Connector.rename = function(params, callback) {
	var old = params.old;
	old = old.replace(/\/+$/, "");
	console.log(old);
	var n = params.new;
	...
	Connector.sync(callback.bind(null, {
		...
		"Old Path" : old,
		"New Path" : n,
		"Old Name" : old.split("/").pop(),
		"New Name" : n.split("/").pop()
	}));
}

Connector.getinfo = function(params, callback) {
	callback(Connector.getInfoSync(params.path));
}

Connector.getfolder = function(params, callback) {
	var path = params.path;
	var directories = {};
	FS.readdir(path).forEach(function(entry) {
		if (entry != "." && entry != "..") {
			var realpath = path + entry;
			directories[realpath] = Connector.getInfoSync(realpath);;
		}
	});
	callback(directories);
}

Connector.savefile = function (params, callback, data) {
	var content = data[0].value;
	var path = data[2].value;
...
Connector.addfolder = function(params, callback) {
	var fullpath = params.path + params.name;
	FS.mkdir(fullpath);
	Connector.sync(callback.bind(null, {
		"Error" : "No error",
		"Code" : 0,
...
Connector.getJSON = function(query, callback) {
	query = query.substring(query.indexOf("?") + 1);
	var args = getJsonFromUrl(query);
	var mode = args.mode;
	delete args.mode;
	console.log(arguments);
	Connector[mode](args, callback);
};

Connector.postJSON = function(url, callback, data) {
	Connector.savefile(url, callback, data);
}
...
(function($) {
	...
	var loadConfigFile = function (type) { var json = null;
		...
		$('#newfolder').unbind().click(function(){
			...
			var getFolderName = function(v, m){
				...
				if(fname != ''){
					...
					Connector.getJSON(fileConnector + '?mode=addfolder&path=' + $('#currentpath').val() + '&name=' + foldername + '&time=' + d.getMilliseconds(), function(result){
						if(result['Code'] == 0){
	...
	var renameItem = function(data) {
		...
		var getNewName = function(v, m){
			...
			if(rname != ''){
				...
				Connector.getJSON( connectString,function(result){
						if(result['Code'] == 0){
			...
			Connector.getJSON(connectString, function(result) {
					if (result['Code'] == 0) {
						...
						$('#edit-save').click(function() {
							...
							Connector.postJSON(fileConnector, function(result){
								if(result['Code'] == 0){
									isEdited = true;
	...
	var setMenus = function(action, path) {
		var d = new Date(); // to prevent IE cache issues
		Connector.getJSON(fileConnector + '?mode=getinfo&path=' + path + '&time=' + d.getMilliseconds(), function(data){
			if($('#fileinfo').data('view') == 'grid'){
	...
	var getFileInfo = function(file) {
		...
		var d = new Date(); // to prevent IE cache issues
		Connector.getJSON(fileConnector + '?mode=getinfo&path=' + encodeURIComponent(file) + '&time=' + d.getMilliseconds(), function(data){
			if(data['Code'] == 0){
	...
	var getFolderInfo = function(path) {
		...
		Connector.getJSON(url, function(data){
			var result = '';
	...
	$(function(){
		...
		if(config.upload.multiple) {
			...
			loadCSS('./scripts/dropzone/build/build.css');
			loadJS('./scripts/dropzone/build/build.js');
